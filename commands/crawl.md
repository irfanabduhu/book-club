...
The presentation weaves articles into thematic **nets** — clusters of articles that develop a shared idea — and presents them as a coherent intellectual map rather than a chronological list.
...
Report to the user: "Found N articles on [Author]'s page. Grouping into thematic nets..."

---

## Phase 2: Map & Group into Nets

### Classify articles into nets from titles and descriptions alone:
...
2. **Create 3-7 nets**, each with:
   - A **net name** — a descriptive phrase capturing the intellectual thread (e.g., "The Startup Founder's Mind", "Programming as Thinking", "Wealth and Ambition")
   - A **one-sentence description** of what connects the articles in this net
   - The **list of articles** belonging to this net (by title and URL)
3. **Allow overlap** — an article can appear in multiple nets if it genuinely bridges themes
4. **Create an "Uncategorized" net** for articles that don't fit any clear pattern — but try to minimize this

### Net quality tests:
- Each net should have **at least 3 articles**. Merge smaller clusters.
- Each net should have a **coherent intellectual thread**, not just a shared topic. "Essays about X" is a topic; "How the author's thinking about X evolved from A to B" is a thread.
- The net name should be evocative, not generic. "Technology" is bad; "Tools Shape the Thinker" is good.

### Present nets to user:
Show the user the discovered nets with article counts and descriptions. Ask if they want to:
- Include all nets
- Select specific nets
- Merge or split any nets
...
### For each selected net, fetch the full articles:

Launch **parallel agents per net** (one agent per net, or split large nets across multiple agents). Each agent:

1. Fetches the full text of each article in its net using WebFetch
...
4. Records for each article:
   ...
   - How it **relates to other articles** in the net — does it build on, contradict, refine, or echo ideas from other pieces?
...
### For each net, select excerpts and plan the narrative:

1. **Select 5-12 excerpts per net** from the candidate pool. Favor passages that:
   - State the core idea of the net most powerfully
...
2. **Order excerpts within each net** by **conceptual progression**, not chronology. Start with the most accessible or foundational formulation, build toward the most mature or provocative. Use dates to show evolution, but don't let chronology dictate flow.

3. **Plan cross-net connections**: Identify 2-3 moments where a passage in one net illuminates or connects to a passage in another net. These will become cross-reference margin notes.

### Selection quality tests:
1. **The thread test**: Does this excerpt advance the net's intellectual thread, or is it just a good passage that happens to be in this net?
...
- **Cross-net connections** — "This connects to the [Net Name] thread — the same instinct expressed differently"
...
### At least 25% of margin notes should make cross-article or cross-net connections. This is what makes a crawl more than a reading list.
...
For crawls with 4+ nets, use chunked generation:
1. Generate structural slides (title, map, net transitions, discussion, closing) directly
2. Launch parallel agents per net to generate content slides
...
3. **Replace placeholders**:
   ...
   - `{{BOOKS_SUBTITLE}}` → number of nets (e.g., "A Web in Five Nets")
4. **Populate slides** inside `<div class="slides-wrapper">`:

### Slide structure:

- **Slide 0 — Title**: `class="slide active no-margin"` — the crawl title, author name, number of nets, a unifying quotation from the writings. Uses `study-title-page` class. Includes `begin-hint`.

- **Slide 1 — The Map**: `class="slide no-margin"` — overview of all nets in the crawl. Uses `.book-card` elements repurposed for nets: each shows the net name, article count, and one-sentence description. Nets listed in the presentation order.

- **Slide 2 — Roadmap**: `class="slide no-margin"` — the author's intellectual landscape in brief. A visual diagram showing how the nets relate to each other.

- **Slides 3 through N — Content slides** (the bulk): A mix of:
  - **Excerpt slides** (`class="slide"`): Direct article excerpts with margin notes. The `page-chapter` label includes the article title: `"Article Title &middot; Year"`. Use `data-book` attribute with the net number for color coding.
  ...
  - **Net transition slides** (`class="slide no-margin book-transition"`): Brief orienting slide when focus shifts to a new net. Shows net name, article count, and a one-sentence framing. Uses the net's assigned color.
  - **Thread slides** (`class="slide no-margin"`): Synthesis slides that analyze a cross-net pattern.

- **Slide N+1 — Discussion**: `class="slide no-margin"` — 6-8 questions, prioritizing questions that span nets or trace the author's evolution.
...
- Use `data-book` attribute to map nets to the study template's book color system (net 1 = `data-book="1"`, etc.)

### Slide count guidance:
- **2-3 nets**: 20-35 total slides
- **4-5 nets**: 30-45 total slides
- **6-7 nets**: 40-55 total slides
...
- **Nets, not lists**: The value is in the thematic grouping and the connections between articles. A chronological list of excerpts is not a crawl.
...
- **The margin notes are the value-add**: Cross-article and cross-net margin annotations — showing how an idea from a 2005 essay connects to a 2019 essay — are what make this a *crawl* rather than a reading list.
- **Not all nets need equal weight**: A net with 3 articles on a minor theme might get 3-4 excerpt slides. A net with 15 articles on the author's central obsession might get 10-15.
